	...
	function mailinglist($export = 0){
		...
		if(!empty($sqlRes)){
			foreach($sqlRes as $lineRes){
				...
				$mydata[$lineRes->listid]['nbOpenRatio'] = $lineRes->nbOpen / $mydata[$lineRes->listid]['nbHtml'] * 100;
				$mydata[$lineRes->listid]['nbBounce'] = $lineRes->nbBounce;
				$mydata[$lineRes->listid]['nbBounceRatio'] = $lineRes->nbBounce / $mydata[$lineRes->listid]['nbMailSent'] * 100;
			}
		} else{
		...
		if(!empty($sqlRes)){
			foreach($sqlRes as $lineRes){
				...
				$mydata[$lineRes->listid]['nbClicRatio'] = $lineRes->nbClic / $mydata[$lineRes->listid]['nbHtml'] * 100;
			}
		}
		...
		if(!empty($sqlRes)){
			foreach($sqlRes as $lineRes){
				...
				$mydata[$lineRes->listid]['nbUnsubRatio'] = $lineRes->nbUnsub / $mydata[$lineRes->listid]['nbMailSent'] * 100;
			}
		}
