...
class KideControllerKide extends KController {
	function display() {
		parent::display();
	}
	...
	function sesiones() {
		...
		if (JRequest::getInt('show_sessions')) {
			...
			echo '<xml>';
			$db->setQuery("SELECT * FROM #__kide_sesion WHERE ocultar=0 AND userid!=0  AND time>".(time() - $params->get("session_time", 200)). " ORDER BY name ASC");
	...
	function add() {
		...
		if ($kuser->rango == 3 && !$kuser->captcha) {
			echo '<xml></xml>';
			exit;
		}
		if ($kuser->rango == 4) {
		...
		if ($kuser->checkBan()) {
			$db->setQuery("DELETE FROM #__kide_bans WHERE time<".time());
			$db->query();
			$db->setQuery('INSERT INTO #__kide (name,userid,text,time,color,rango,sesion,token,img,url) VALUES ("System", 0, "'.str_replace("%name", $kuser->name, JText::_("COM_KIDE_USER_BANEADO")).'", '.time().', "'.$params->get('color_sp', '000').'", 0, 0, 0, "", "")');
			$db->query();
			echo '<xml banned="1"></xml>';
			exit;
		}
		...
		if ($txt && $txt != JText::_("COM_KIDE_NOSPAM")) {
			...
			$db->setQuery('INSERT INTO #__kide (name,userid,text,time,color,rango,token,sesion,img,url,ip) VALUES ("'.$kuser->name.'", '.$kuser->id.', "'.addslashes($txt).'", '.time().', "'.$kuser->color.'", '.$kuser->rango.', '.$kuser->token.', "'.$kuser->sesion.'", "'.$kuser->img.'", "'.kideLinks::getUserLink($kuser->id).'", "'.$_SERVER['REMOTE_ADDR'].'")');
			$db->query();
			
			$db->setQuery("SELECT id FROM #__kide WHERE token=".$kuser->token." ORDER BY id DESC");
			$id = $db->loadResult();
