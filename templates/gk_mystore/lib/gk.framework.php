...
class GKTemplate {
    ...
    */
    ...
    public function generatePadding($block) {
    	...
    	if($block == 'gkLeftLeft') return 'gkPaddingR';
    	if($block == 'gkLeftRight') return 'gkPaddingR';
		...
		if($block == 'gkRightLeft') return ($this->modules('right_right')) ? 'gkPaddingTR' : 'gkPaddingTBLR';
		if($block == 'gkRightRight') return 'gkPaddingTL';
		...
		if($block == 'gkContentMainbody') {
			if($this->modules('right_top + right_bottom + right_left + right_right')) {
				$this->addCSSRule('#gkContent { padding-right:16px !important; }');
			}
